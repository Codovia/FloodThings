...
class TestOpenMeteoAdapterSeparation:
    """Test strict separation of weather observations vs weather forecasts."""
    ...
    def test_operational_payload_separation(self):
        ...
        with factory() as session:
            ...
            res = adapter.ingest(
                mode="operational",
                locations=[{"name": "Test Loc", "lat": 12.9716, "lon": 77.5946, "district_code": "526"}],
                client=mock_client,
            )

            assert res.status in ("SUCCESS", "PARTIAL")

            # Verify past record went to WeatherObservation & RainfallObservation
            past_dt = datetime(2020, 1, 1, 10, 0, tzinfo=timezone.utc)
            w_obs = session.query(WeatherObservation).filter_by(observed_at=past_dt).first()
            assert w_obs is not None
            assert w_obs.temperature == 26.5
            assert w_obs.wind_speed == 3.5

            r_obs = session.query(RainfallObservation).filter_by(observed_at=past_dt).first()
            ...
            fcst = session.query(WeatherForecast).filter_by(forecast_for=future_dt).first()
